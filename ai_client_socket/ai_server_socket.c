...
typedef struct {
    ...
    MY_RECORDER_CTX_S my_recorder_ctx_s;
} ClientData;
...
static RK_S32 upload_audio_to_socket_server(ClientData *client) {
    char log_msg[256];
    MY_RECORDER_CTX_S *ctx = &(client->my_recorder_ctx_s);
    ...
    if (send_config_message(new_socket, ctx->responseFormat) != RK_SUCCESS) {
...
static void* recording_thread(void *ptr) {
    ClientData * client = (ClientData*)(ptr);
    MY_RECORDER_CTX_S *ctx = &(client->my_recorder_ctx_s);
    ctx->sockfd = client->socket;
    int new_socket = client->socket;
    RK_S32 result = 0;
    RK_S32 s32MilliSec = -1;
                ...
                if (fp) {
                    ...
                    fflush(stdout);
                     ...
                     if (ctx->s32EnableUpload) {
                         ...
                         fflush(stdout);
                         upload_audio_to_socket_server(client);
                         //send(new_socket, bye_msg, strlen(bye_msg), 0);
...
int mainclient(MY_RECORDER_CTX_S * ctx) {
...
void *handle_client(void *arg) {
    ...
    MY_RECORDER_CTX_S *ctx = &(client->my_recorder_ctx_s);
    int res = mainclient(ctx);
    printf("mainclient:res:%d",res);
    char buffer[BUFFER_SIZE] = {0};
    
    // 打印客户端信息
    printf("新客户端连接: %s:%d (线程ID: %ld)\n", 
    ...
    while (1) {
        ...
        printf("来自 %s:%d 的消息: %s\n", 
               ...
               ntohs(address.sin_port), 
               buffer);
        pthread_t recordingThread,gpioThread;
        // 创建录音线程
        pthread_create(&recordingThread, NULL, recording_thread, (void *)client);    
        pthread_detach(recordingThread);
        // 检查是否是退出命令
        if (strncmp(buffer, "startrecord", 11) == 0) {
            const char *bye_msg = "ok,started";
            send(new_socket, bye_msg, strlen(bye_msg), 0);
            //处理开始录音的指令
            pthread_create(&gpioThread, NULL, gpio_monitor_thread, (void *)client);
            pthread_detach(gpioThread);
        }   
        if (strncmp(buffer, "stoprecord", 10) == 0) {
            const char *bye_msg = "ok,stoped";
            send(new_socket, bye_msg, strlen(bye_msg), 0);
            //处理结束录音的指令
        }   
        ...
        snprintf(response, BUFFER_SIZE, "已收到: %s", buffer);
